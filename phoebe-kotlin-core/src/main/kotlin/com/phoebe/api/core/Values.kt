...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String =
        asString() ?: throw PhoebeInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw PhoebeInvalidDataException("`$name` is not set")
            is JsonNull -> throw PhoebeInvalidDataException("`$name` is null")
            else -> throw PhoebeInvalidDataException("`$name` is invalid, received $this")
        }
